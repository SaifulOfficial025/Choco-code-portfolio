  ...
  return (
    <div className="flex flex-col items-center w-full relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full h-[400px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-[320px] md:h-[400px] overflow-hidden rounded-2xl shadow-xl border border-black/10">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || card.name || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent z-20 pt-10 text-center">
                  <p className="text-white font-bold font-serif text-lg">{card.name}</p>
                  <p className="text-cc-yellow text-xs font-mono mt-1">{card.role}</p>
                </div>
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer w-[200px] md:w-[260px] absolute">{image}</a>
            ) : (
              <div key={index} className="fan-card w-[200px] md:w-[260px] absolute">{image}</div>
            );
          })}
        </div>
      </div>
